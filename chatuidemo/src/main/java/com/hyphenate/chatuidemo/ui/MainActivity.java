...
import com.hyphenate.chatuidemo.redpacket.http.HttpResultCallback;
...
public class MainActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null && savedInstanceState
				.getBoolean(Constant.ACCOUNT_REMOVED, false)) {
		...
		if (!TextUtils.isEmpty(currentUserName)) {
			...
			config.groupType = 3;
			String username = RPHelper.getCurrentUser();
			EaseUser user = RPHelper.getEaseUser(username);
			if (user != null) {
	...
	private static int authTryTime = 3;
	...
	private void auth() {
		...
		DialogUtils.showNetProgress(this, null);
		RequestUtils.requestByHttpGet(
				RequestUtils.SIGN_BASE_URL + currentUserName,
				new HttpResultCallback() {
					@Override
					public void onSuccess(String result) {
						DialogUtils.dismissProgress();
						if (TextUtils.isEmpty(result)) {
							Toast.makeText(MainActivity.this, "获取签名失败",
				...
				.requestAllManifestPermissionsIfNecessary(this,
						new PermissionsResultAction() {
							...
							public void onGranted() {
								// Toast.makeText(MainActivity.this, "All
	...
	private void initView() {
		...
		unreadAddressLable = (TextView) findViewById(
				R.id.unread_address_number);
		mTabs = new Button[3];
	...
	EMMessageListener messageListener = new EMMessageListener() {
		...
		public void onCmdMessageReceived(List<EMMessage> messages) {
			...
			for (EMMessage message : messages) {
				...
				// if
				// (action.equals(RPConstant.REFRESH_GROUP_RED_PACKET_ACTION)) {
	...
	private void registerInternalDebugReceiver() {
		...
		IntentFilter filter = new IntentFilter(
				getPackageName() + ".em_internal_debug");
