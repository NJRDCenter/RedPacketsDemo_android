package com.hyphenate.chatuidemo.redpacket.adapter;
...
public class MembersChooseAdapter extends ArrayAdapter<EaseUser> implements
		SectionIndexer {
	...
	private List<String> mCheckedUserList;

	public MembersChooseAdapter(Context context, int resource,
		...
		mCheckedUserList = new ArrayList<>();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			if (res == 0)
				convertView = layoutInflater.inflate(
						R.layout.em_listitem_members_choose, parent, false);
			else
			...
			holder.stateCb = (CheckBox) convertView
					.findViewById(R.id.em_cb_member_item);
		...
		if (mCheckedUserList.contains(user.getUsername())) {
	...
	public MembersChooseAdapter setPrimaryColor(int primaryColor) {
		this.primaryColor = primaryColor;
		return this;
	}

	public MembersChooseAdapter setPrimarySize(int primarySize) {
		this.primarySize = primarySize;
		return this;
	}

	public MembersChooseAdapter setInitialLetterBg(Drawable initialLetterBg) {
		this.initialLetterBg = initialLetterBg;
		return this;
	}

	public MembersChooseAdapter setInitialLetterColor(int initialLetterColor) {
	...
	public void clickItem(int position) {
		EaseUser easeUser = getItem(position);
		if (easeUser == null || TextUtils.isEmpty(easeUser.getUsername())) {
			return;
		}
		if (mCheckedUserList.contains(easeUser.getUsername())) {
			mCheckedUserList.remove(easeUser.getUsername());
			notifyDataSetChanged();
		} else if (mCheckedUserList.size() < 5) {
			mCheckedUserList.add(easeUser.getUsername());
			notifyDataSetChanged();
		}
	}
	 ...
	 * @return 选中用户列表
	 */
	public List<EaseUser> getCheckedEaseUsers() {
		List<EaseUser> easeUserList = new ArrayList<>();
		for (String username : mCheckedUserList) {
